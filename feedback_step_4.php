...
$sql=mysqli_query($conn,"select * from feeds where roll='".mysqli_real_escape_string($conn,$_POST['roll'])."' AND name='".mysqli_real_escape_string($conn,$_POST['faculty'])."' AND subject='".mysqli_real_escape_string($conn,$_POST['subject'])."'");
...
if(mysqli_num_rows($sql)>0)
{
    ...
    <?php
    write_log("Feedback already submitted");
}
...
//Fetch Questions
$q = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM questions WHERE id = '1'"));
$parameters = array("Strongly Disagree","Disagree","Not Sure","Agree","Strongly Agree");
?>
<!doctype html>
